...
def playAssistantSound():
    music_dir = os.path.join("www", "assets", "audio", "cat-meowing-ai-generated-261841.mp3")
    try:
        pygame.mixer.init()
...
def geminai(query):
    try:
        ...
        # Check if query needs real-time/live data
        realtime_keywords = [
            'weather', 'temperature', 'forecast',
            'news', 'current affairs', 'today', 'latest', 'right now', 'live',
            'share market', 'stock market', 'nifty', 'sensex', 'bse', 'nse',
            'forex', 'exchange rate', 'dollar', 'euro', 'currency',
            'cricket', 'score', 'match', 'ipl',
            'price', 'gold price', 'silver price', 'petrol', 'diesel',
            'trending', 'breaking', 'update'
        ]

        needs_search = any(word in query.lower() for word in realtime_keywords)

        if needs_search:
            print(f"[Gemini] Using web search for: {query}")
            try:
                # Try with grounding/search tool
                ...
                print(f"[Gemini] Search tool failed: {search_err}, falling back to normal")
                response = client.models.generate_content(
                    model="gemini-2.5-flash",
                    contents=query
                )
        else:
            print(f"[Gemini] Normal response for: {query}")
            response = client.models.generate_content(
                model="gemini-2.5-flash",
                contents=query
            )
        ...
        if not result_text:
            result_text = "Sorry Sir, I could not find information on that."

        filter_text = markdown_to_text(result_text)
        speak(filter_text)
